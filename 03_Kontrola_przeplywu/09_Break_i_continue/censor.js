...
function censor(phrasesToCensor) {
  ...
  for (const prop in phrasesToCensor){
    if (phrasesToCensor[prop].includes('bloop' === true)  ){
      continue
    }
     if (phrasesToCensor[prop].includes('bleep' === true)  ){
      break
    }
     else {
       censoredPhrases.push(phrasesToCensor[prop])
     }
  }
